...
class HomeCategoriesTests(DesignFixtures):
    ...
    def test_any_signed_in_user_can_load_it_not_only_admins(self):
    ...
    def test_categories_come_in_the_order_of_the_category_list(self):
        ...
        self.assertEqual(
            self._labels(self._get()), [category['label'] for category in listed.data],
        )
    ...
    def test_a_category_carries_its_picture_and_priority(self):
        ...
        self.assertEqual(beds['id'], self.beds.id)
        self.assertEqual(beds['priority'], 3)
        self.assertIn('demo-cloud', beds['image_url'])
        # Read-only here: nothing about editing or deleting.
        self.assertNotIn('image', beds)
    ...
    def test_the_design_count_is_the_total_not_just_what_is_listed(self):
        ...
        beds = self._by_label(self._get(per_category=2))['Beds']

        self.assertEqual(beds['design_count'], 5)
        self.assertEqual(len(beds['designs']), 2)

    def test_the_counts_match_the_category_list(self):
        for i in range(3):
            self._design(f'Bed {i}', self.beds)
    ...
    def test_each_category_carries_only_its_own_designs(self):
        self._design('Headboard', self.beds)
        self._design('Bed frame', self.beds)
        self._design('Front door', self.doors)
        ...
        self.assertEqual(sorted(self._titles(categories['Beds'])), ['Bed frame', 'Headboard'])
        self.assertEqual(self._titles(categories['Doors']), ['Front door'])

    def test_designs_are_newest_first(self):
        for title in ('First', 'Second', 'Third'):
            self._design(title)
    ...
    def test_the_limit_is_the_newest_few(self):
        ...
        self.assertEqual(self._titles(beds), ['Bed 4', 'Bed 3'])

    def test_the_limit_applies_to_each_category_on_its_own(self):
        for i in range(3):
            self._design(f'Bed {i}', self.beds)
            self._design(f'Door {i}', self.doors)

        categories = self._by_label(self._get(per_category=2))

        self.assertEqual(self._titles(categories['Beds']), ['Bed 2', 'Bed 1'])
        self.assertEqual(self._titles(categories['Doors']), ['Door 2', 'Door 1'])

    def test_a_busy_category_does_not_crowd_out_another(self):
        for i in range(30):
            self._design(f'Bed {i}', self.beds)
        self._design('Only door', self.doors)

        categories = self._by_label(self._get())

        self.assertEqual(self._titles(categories['Doors']), ['Only door'])

    def test_the_limit_can_be_the_smallest_and_the_largest_allowed(self):
        self._design('Headboard')

        self.assertEqual(self._get(per_category=1).status_code, 200)
        self.assertEqual(self._get(per_category=50).status_code, 200)

    def test_a_limit_out_of_range_or_not_a_number_is_a_400(self):
        for bad in ('0', '51', '-1', 'abc', '1.5', '1;drop', '\u00b2'):
            with self.subTest(per_category=bad):
                response = self._get(per_category=bad)
                self.assertEqual(response.status_code, 400)
                self.assertIn('per_category', response.data)

    def test_an_empty_limit_means_the_default(self):
        for i in range(25):
    ...
    def test_a_design_has_what_a_card_and_the_details_screen_need(self):
        sub = SubCategory.objects.create(category=self.beds, label='King')
        design = self._design('Headboard', sub_category=sub, design_file=True)

        item = self._by_label(self._get())['Beds']['designs'][0]
    ...
    def test_it_never_exposes_the_private_cutting_file(self):
        ...
        item = self._by_label(response)['Beds']['designs'][0]
        self.assertIsNone(item['design_file_url'])
        self.assertTrue(item['has_design_file'])
        self.assertNotIn(design.design_stored_file.storage_key, response.content.decode())
    ...
    def test_the_number_of_queries_does_not_grow_with_the_catalogue(self):
        sub = SubCategory.objects.create(category=self.beds, label='King')
        ...
        for i in range(2):
            self._design(f'Early bed {i}', self.beds, sub_category=sub, design_file=True)
        few = queries()

        for n in range(4):
            category = Category.objects.create(label=f'Extra {n}')
            for i in range(6):
                self._design(f'Extra {n} design {i}', category, design_file=True)
        many = queries()
